 ...
 ** Computer Interface
 *
 **** Always the same int yourNumber int numPlayers int numPlanets int mapWidth
 * int mapHeight String[numPlayers] playerNames *** Vary from turn to turn int
 * time (in segments) int numMessages (number of messages to this player this
 * turn) int[numMessages] messageSender String[numMessages] messageText **
 * Planet Info int[numPlanets] planetX (x positions of all planets)
 * int[numPlanets] planetY (y positions of all planets) int[numPlanets]
 * planetOwner (owner of planet) int[numPlanets] planetShips (ships on planets)
 * int[numPlanets] planetProduction (production on planets) int[numPlanets]
 * planetRatio (ratio on planets) int[numPlanets] planetYourAttackers (number of
 * your attackers on planet) int[numPlanets] planetOtherAttackers (number of
 * other attackers on planet) ** Fleet Info int numFleets (number of Arriving
 * fleets) int[numFleets] fleetOwner (owner of fleets) int[numFleets]
 * fleetDestination (destination planets of fleets) int[numFleets] fleetShips
 * (number of ships on fleets) int[numFleets] fleetRatio (ratios of fleets)
***
 */
// import igx.shared.*;
import igx.bots.*;
...
public class ComputerInterface extends Bot {
    ...
    public native ComputerResult beejify(int yourNumber,
            int numPlayers,
            int numPlanets,
            int mapWidth,
            int mapHeight,
    ...
    public void update(GameState game, GameState old, Message[] message) {
	  ...
	  gameOver = getSkillLevel() + 1;*/
        ...
        ComputerResult r;
        monitor.lock();
        r = beejify(yourNumber, numPlayers, numPlanets, mapWidth, mapHeight, playerNames, time, numMessages, messageSender, messageText, planetX, planetY, planetOwner, planetShips, planetProduction, planetRatio, planetYourAttackers, planetOtherAttackers, numFleets, fleetOwner, fleetDestination, fleetShips, fleetRatio, gameID, gameOver);
        monitor.unlock();
