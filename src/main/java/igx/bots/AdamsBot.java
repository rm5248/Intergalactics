...
/**
	Written by Peter Maxwell 17/9/99
	with help from Keith Houston
												**/
		...
		for( int i = 0; i < Constants.PLANETS; i++) 
	 	{
			...
			if( p.getOwner() == BotNumber && distance < NearestfriendDistance )
				{ Nearestfriend = i; NearestfriendDistance = distance; continue; }
			if( p.isNeutral() && distance < NearestneutralDistance )
				{ Nearestneutral = i; NearestneutralDistance = distance; continue; }
			if( p.getOwner() != BotNumber && distance < NearestenemyDistance )
				{ Nearestenemy = i; NearestenemyDistance = distance; continue; }
		}
