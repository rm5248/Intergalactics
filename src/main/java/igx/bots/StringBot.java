 ...
 * <ratio> times as many ships, and attack it.  Otherwise, pool
 * resources and wait.
 *
 * @author Bill Baker
 */
public class StringBot extends Bot
{
    ...
    * attacking planet should send a fleet to the defending planet, indexed
    * by skill level */
   static final double[] OVERWHELMING =   { 3.5, 5, 2.5,   5, 3.5, 3.5, 2.5, 3.5, 5 };

   /**
    * the ratio of the hypothetical attack fleet's strength to the
    * defending planet's strength, indexed by skill level */
    ...
    * production, indexed by skill level -- for example, if the
    * GARRISON_RATIO is 1.7 and production is 10, then the number of ships
    * to keep is 17. */
   static final double[] GARRISON_RATIO = {   1, 2,  .8, 1.4,  .7, 1.2,   2,   2, 2 };

   /**
    * The minimum number of ships to attack with (sometimes the
    * attacking fleet size calculation will come up with 0).  */
   ...
   public String createName (int skillLevel) { 
      ...
      while (index >= STRING_NAMES.length) index -= STRING_NAMES.length;
      while (index < 0) index += STRING_NAMES.length;
   ...
   /**
    * The method is called when a new game is begun. The StringBot
    * doesn't need to do anything here, but other 'bots might find use
    * for this.  */
   ...
   /**
    * This function is overridden so there are only five StringBots
    * (Only five skill levels).  */
   public int numberOfBots () { return 6; }
   ...
   /**
    * Defensive strength if we send a fleet now: the square of the
    * number of ships times the defense ratio at the time that an
    * attack would occur if a fleet left now from pAtt to attack pDef
    * -- assume expected value of max defense bonus and assume that no
    * additional reinforcements will arrive */
   ...
   /**
    * How many ships do we expect at a certain time at a certain
    * planet?  Calculated as current ships + generated there between
    * now and then.  */
   public int expectedShips(int iPlanet, int time) {
      int dt = time - curGame.getTime();
      Planet p = curGame.getPlanet(iPlanet);
      if (p.isNeutral()) return neutralShips;
      else
	 return p.getShips() + p.getProduction() * dt / Constants.SEGMENTS; }

   /**
    * Offensive strength for a fleet of a certain size, attacking from
    * planet iAtt */
   ...
   /**
    * the time, in segments, that it would take for a fleet to get
    * between planets p1 and p2 */
    ...
    * <li>if the closest enemy planet is closer than the closest
    *     friendly planet with a better attack ratio then, <ul>
    *     <li>if we have more than a certain number of ships
    *         available, attack that enemy planet
    *     <li>otherwise sit and wait (we could do something more
    *         sophisticated though, mind you)
    *     </ul>
    * <li>otherwise, send a certain number of ships off to that better
    *     friendly planet
    * </ul>
    */
   public void update (GameState game,
	 ...
	 */
	 ...
	 if (ourP.getOwner() != me) continue;
	 // if under attack ...
	 int numAttackers = ourP.getTotalAttackers();
	 if (numAttackers > 0) {
	    // ... if we're outnumbered (DEBUG to test if we're
	    // outmatched), clear out ...
	    if (numAttackers > ourP.getShips()) bugOut(i);
	    // ... but if it looks good, sit tight at this planet
	    // until the battle is over
	 }
	 // if not under attack, distribute resources sans panic
	 else i = thinkCalmly(i);
      }
   }

   /**
    * Would sending all ships from sourceP to destP be a wise attack?
    * That is, would our offensive strength be greater than or equal
    * to ATTACK_RATIO times their defensive strength?  */
   public boolean attackGoodIdea(int sourceP, int destP) {
      return attackGoodIdea
	 (sourceP, destP, curGame.getPlanet(sourceP).getShips()); }
   ...
   /**
    * Let's get outta here - either to a safe place or to a planet we
   ...
   public void bugOut(int iOurP) {
	 ...
	 // if it's an enemy's, attack if we can win
	    ...
	    if (attackGoodIdea(iOurP, jClosest)) {
   ...
   /**
    * Decide if and where to send ships from this planet.  Return the
    * current planet to consider (return iOurP means go on,
    * Constants.PLANETS means we're done).  */
   protected int thinkCalmly(int iOurP) {
	 ...
	 if (enemyP.getOwner() == me) continue;
	 // skip planets that we are already attacking
	 if (attacks.containsKey(new Integer(j))) continue;
	 int d2 = dist2(enemyP, ourP);
	 if (enemyDist2 == 0 || d2 < enemyDist2) {
	    enemyDist2 = d2; closestEnemyP = enemyP; jClosestEnemy = j; } }
      /*
	...
	Planet.planetChar(jClosestEnemy));
	 ...
	 if (k == iOurP) continue; // skip current planet (ourP)
	 ...
	 if (qP.getRatio() > ourP.getRatio()) { // better ratio?
	    ...
	    if (betterMinDist2 == 0 || // haven't found one yet or ...
		qDist2 < betterMinDist2 || // ... closer or ...
		(qDist2 == betterMinDist2 && // ... same dist and better
		 qP.getRatio() > betterP.getRatio())) {
	       kBetterP = k; betterP = qP; betterMinDist2 = qDist2; }
	    // best ratio that is closer than closest enemy
	    if (qDist2 < enemyDist2 && qP.getRatio() > ourP.getRatio()) {
	       if (bestP == null || // if haven't found one yet, ...
		   // ... or ratio is better ...
		   bestP.getRatio() < qP.getRatio() ||
		   // ... or ratio is same and distance is less
		   (bestP.getRatio() == qP.getRatio() && 
		    qDist2 < bestMinDist2)) {
		  kBestP = k; bestP = qP; bestMinDist2 = qDist2; } } }
      }
      /*
	if (betterMinDist2 != 0)
	...
	Planet.planetChar(jClosestEnemy) + " = " + defStr);
      */
      ...
      if (ourP.getRatio() == 0 || // prevent DivBy0 in else{} below
	 ...
	 if (shipsToSend <= 0) shipsToSend = MIN_ATTACK;
   ...
   /**
    * send an attack fleet of numShips ships from sourcePlanet to
    * destPlanet */
